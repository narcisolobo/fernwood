...
- Run lint and clean up temporary/debug code — specifically confirm the `referenceWidth`/`naturalTargetWidth` `console.log`s added while debugging `TrackedToMatch`'s width-measurement bug were actually removed, not just assumed gone

### 7. Lighthouse audit — required before deployment

- Run in Chrome Incognito to get an unbiased baseline across Performance, Accessibility, Best Practices, and SEO; fix whatever it flags before a reviewer finds it first

### 8. Reset/clean seed data before launch

- Real personal data (`ciso@cisocodes.com` and associated test bookings/waitlist entries) is currently sitting in the live database from manual testing. Not a security exposure — that table isn't publicly readable — but worth a deliberate decision about resetting to clean seed data before pointing reviewers at the live site.
...
- Postgres doesn't auto-index foreign key columns (`class_id`, `student_id`, `instructor_id`). Correct, standard practice — but genuinely premature at current data scale (a few dozen rows); worth adding once real traffic/data volume justifies it, not as a launch blocker.
...
- Confirm all images go through `next/image` with explicit sizing, and custom fonts load via `next/font` — likely already correct given how the site was built, but worth an explicit verification pass rather than assuming.

### 6. README schema/ERD summary

- A visual database schema diagram to accompany the existing architecture notes — nice addition, not blocking, since the security model is already documented in prose.
